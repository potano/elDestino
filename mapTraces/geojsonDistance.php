...
define('METERS_TO_FEET', 100/(2.54*12));
define('METERS_TO_MILES', METERS_TO_FEET / 5280);
      ...
      if (($doc[0] == 'path' || $doc[0] == 'polygon') && is_float($doc[$next])) {
         ...
         foreach (array_slice($doc, $next) as $i => $val) {
            ...
            if ($odd) {
               $long = $val;
               if ($prevLong) {
                  $dist += haversineDistance($prevLat, $prevLong, $lat, $long);
                  $tail = sprintf("\t; %6.0fm   %3.1fm", $dist, $dist * METERS_TO_MILES);
               }
               $prevLat = $lat;
               $prevLong = $long;
            }
...
function haversineDistance($lat1, $long1, $lat2, $long2) {
   $lat1 *= DEG_TO_RADIANS;
   $long1 *= DEG_TO_RADIANS;
   $lat2 *= DEG_TO_RADIANS;
   $long2 *= DEG_TO_RADIANS;

   $sinDlat = sin(($lat2 - $lat1) / 2);
   $sinDlong = sin(($long2 - $long1) / 2);
   $a = $sinDlat * $sinDlat + cos($lat1) * cos($lat2) * $sinDlong * $sinDlong;
